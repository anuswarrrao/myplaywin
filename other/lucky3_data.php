...
// Prepare SQL query
if ($view === 'previous') {
    $stmt = $conn->prepare("SELECT * FROM daily_games WHERE game_name = 'Lucky3' LIMIT ? OFFSET ?");
    $stmt->bind_param("ii", $rowsPerPage, $offset);
    $countStmt = $conn->prepare("SELECT COUNT(*) AS total FROM daily_games WHERE game_name = 'Lucky3'");
} else {
    $stmt = $conn->prepare("SELECT * FROM daily_games WHERE game_name = 'Lucky3' AND DATE(game_time) = ? LIMIT ? OFFSET ?");
    $stmt->bind_param("sii", $currentDate, $rowsPerPage, $offset);
    $countStmt = $conn->prepare("SELECT COUNT(*) AS total FROM daily_games WHERE game_name = 'Lucky3' AND DATE(game_time) = ?");
...
if (empty($rows)) {
    $rows = '<tr><td colspan="2">No Lucky3 games found.</td></tr>';
}
